...
void TIM14_Task(void)
{
	...
	if (rc_Ctrl_et.rc.s1 == 3)Speed_3508 = 0;//左拨杆控制3508上下
   ...
   Yaw_Pitch_Reset();
	...
	if (flag_stop == 0)motor3508.motor[0].Data.Target = Speed_3508;
	else motor3508.motor[0].Data.Target = 0;
...
void up_stop()//上堵转检测，通过或GPIOE4连接微动开关实现，微动开关常闭，堵转时被压下断开，持续一定时间则认为上堵转，3508电机目标角度设定为当前角度-200
{
...
void servo_control()//舵机控制，换弹时黄色舵机置位，发射时红色舵机切换状态
{
...
void LCD_control()//通过LCD模拟比赛两次发射信号
{
...
void  Change_YawPitch_use_remotecontrol()
{

if (abs(rc_Ctrl_et.rc.ch1 - 1024) > 500 && rc_Ctrl_et.rc.s2 == 2)cnt_2006_pitch++;
	else cnt_2006_pitch = 0;
	
	if (abs(rc_Ctrl_et.rc.ch0 - 1024) > 500 && rc_Ctrl_et.rc.s2 == 2)cnt_2006_yaw++;
	else cnt_2006_yaw = 0;
